...
import { BookOpen, Sparkles, ChevronRight, Loader2, Clock, History, Send, ArrowLeft, Zap, Copy, Check, RotateCcw, Plus, Minus, Film, Users, Scissors, Edit3, Save, Trash2, Image, Type, Download, ChevronDown, Search } from 'lucide-react';
...
import { Scene, Character, StoryTheme, StoryScript, CinematicPrompt, IdeaCharacter } from '../types';
...
const StoryStudioModule: React.FC = () => {
  ...
  const [selectedSubTheme, setSelectedSubTheme] = useState<string>(() => loadState('story_selectedSubTheme', ''));
  const [customSubTheme, setCustomSubTheme] = useState<string>(() => loadState('story_customSubTheme', ''));
  ...
  const [ideaCharacters, setIdeaCharacters] = useState<IdeaCharacter[]>(() => loadState('story_ideaCharacters', []));
  const [refinedIdea, setRefinedIdea] = useState<string>(() => loadState('story_refinedIdea', ''));
  ...
  useEffect(() => {
    saveState('story_selectedSubTheme', selectedSubTheme);
  }, [selectedSubTheme]);

  useEffect(() => {
    saveState('story_customSubTheme', customSubTheme);
  }, [customSubTheme]);
  ...
  useEffect(() => {
    saveState('story_ideaCharacters', ideaCharacters);
  }, [ideaCharacters]);

  useEffect(() => {
    saveState('story_refinedIdea', refinedIdea);
  }, [refinedIdea]);
  ...
  const handleSuggestIdea = async () => {
    ...
    try {
      ...
      // Extract characters from idea
      const chars = await extractCharactersFromIdea(idea);
  ...
  const handleRefineIdea = async () => {
    if (!storyIdea || ideaCharacters.length === 0) return;
    setLoading(true);
    try {
      const refined = await refineStoryIdea(selectedTheme, storyIdea, ideaCharacters);
  ...
  const handleDevelopScript = async () => {
    ...
    setLoading(true);
    try {
      const res = await developStoryScript(
        selectedTheme, 
        refinedIdea || storyIdea, 
        numEpisodes, 
        durationPerEpisode,
        undefined, // No product in StoryStudio
        ideaCharacters
      );
  ...
  const handleDownloadAll = () => {
    ...
    let content = `XƯỞNG TRUYỆN: ${script.title}\n`;
    ...
    a.href = url;
    a.download = `prompts-story-${script.title.substring(0, 20).replace(/\s+/g, '-')}.txt`;
              ...
              <div className="space-y-10">
                ...
                <div className="space-y-4">
                  ...
                  {ideaCharacters.length > 0 && (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <label className="luxury-label">Lựa chọn nhân vật</label>
                        <button 
                          onClick={() => setIdeaCharacters([...ideaCharacters, { name: 'Nhân vật mới', gender: 'male', outfit: 'Trang phục bình thường' }])}
                          className="flex items-center gap-2 text-[10px] font-bold text-[#F97316] uppercase tracking-widest hover:text-orange-600 transition-colors"
                        >
                      ...
                      <div className="overflow-x-auto rounded-xl border border-slate-200">
                        <table className="w-full text-left text-xs">
                          <thead className="bg-slate-50 border-b border-slate-200">
                            <tr>
                              <th className="px-4 py-3 font-bold text-slate-500 uppercase tracking-wider">Tên nhân vật</th>
                              <th className="px-4 py-3 font-bold text-slate-500 uppercase tracking-wider">Giới tính</th>
                              <th className="px-4 py-3 font-bold text-slate-500 uppercase tracking-wider">Trang phục</th>
                              ...
                              <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-4 py-2">
                                  <select 
                                    ...
                                    className="bg-transparent border-none focus:ring-0 p-0 text-slate-600 cursor-pointer"
                                  >
                                ...
                                <td className="px-4 py-2">
                                  <input 
                                    ...
                                    className="w-full bg-transparent border-none focus:ring-0 p-0 text-slate-600 italic"
                                    placeholder="Mô tả trang phục..."
                                  />
                                </td>
                                <td className="px-4 py-2">
                                  <button 
                                    onClick={() => setIdeaCharacters(ideaCharacters.filter((_, i) => i !== idx))}
                  ...
                  {refinedIdea && (
                    ...
                    >
                      <div className="flex items-center gap-2 text-[10px] font-bold text-[#F97316] uppercase tracking-widest">
                        <Sparkles className="w-3 h-3" />
                        Ý tưởng đã tinh chỉnh
                      </div>
