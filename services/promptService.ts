...
const JIMENG_SAFETY_GUIDELINES = "\nHƯỚNG DẪN AN TOÀN & CẤM (QUAN TRỌNG CHO JIMENG):\n" +
"1. CHÍNH TRỊ & BIỂU TƯỢNG QUỐC GIA: Tuyệt đối không nhắc tên lãnh đạo, quốc kỳ, quốc huy, bản đồ tranh chấp, sự kiện lịch sử nhạy cảm hoặc biểu tượng tôn giáo cực đoan.\n" +
"2. NỘI DUNG NHẠY CẢM: Không mô tả bộ phận nhạy cảm, hành vi tình dục, trang phục quá hở hang (nội y, xuyên thấu) hoặc tư thế gợi dục.\n" +
"3. BẠO LỰC & KINH DỊ: Không mô tả máu me (gore), chặt chém, xác chết, sự đau đớn tột cùng hoặc quái thai biến dạng. Vũ khí (súng, dao) chỉ dùng trong ngữ cảnh hành động võ thuật/nấu ăn, không dùng cho khủng bố/tấn công cực đoan.\n" +
"4. QUYỀN RIÊNG TƯ: Không dùng tên hoặc mô tả chi tiết để tạo gương mặt giống hệt người nổi tiếng (ca sĩ, diễn viên, KOL). Không mô tả trẻ em trong tình trạng nguy hiểm hoặc không mặc quần áo.\n" +
"5. CHẤT CẤM & PHẠM PHÁP: Không mô tả ma túy, cờ bạc, ngược đãi động vật hoặc bạo hành gia đình.\n";
...
"7. CẤM MÁU VÀ PHỤ ĐỀ: Tuyệt đối không mô tả máu (blood/gore) and không chèn chữ/phụ đề. " +
...
const SALES_STUDIO_INSTRUCTION = "Bạn là một chuyên gia Marketing và biên kịch quảng cáo. " +
"Nhiệm vụ của bạn là tạo ra các prompt video lồng ghép sản phẩm vào một câu chuyện thực tế một cách tự nhiên và hấp dẫn.\n\n" +
"QUY TẮC QUAN TRỌNG:\n" +
"1. SẢN PHẨM LÀ TRỌNG TÂM: Sản phẩm phải xuất hiện một cách hợp lý trong các cảnh quay, giải quyết được vấn đề của nhân vật hoặc làm nổi bật giá trị sử dụng.\n" +
"2. QUY TẮC 3 GIÂY (3-SECOND RULE): Chia nhỏ 12 giây thành các mốc (0-3s, 3-6s, 6-9s, 9-12s). Mỗi mốc mô tả một diễn biến vật lý hoặc tương tác với sản phẩm.\n" +
"3. MÔ TẢ CHI TIẾT SẢN PHẨM: Khi sản phẩm xuất hiện, hãy mô tả rõ ràng: 'Nhân vật cầm [TÊN SẢN PHẨM]', 'Sản phẩm [TÊN SẢN PHẨM] đặt trên bàn', 'Cận cảnh chi tiết [ĐẶC ĐIỂM SẢN PHẨM]'.\n" +
"4. GIỮ NGUYÊN TIẾNG VIỆT: Tên sản phẩm và lời thoại nhân vật trong phần Action PHẢI GIỮ NGUYÊN TIẾNG VIỆT, không được dịch sang tiếng Anh.\n" +
"5. BẢO TỒN CHI TIẾT SẢN PHẨM: Tuyệt đối không được thay đổi chi tiết sản phẩm như hình ảnh sản phẩm, chữ trên sản phẩm, hình dáng sản phẩm.\n" +
"6. ẢNH THAM CHIẾU: Nếu có URL ảnh sản phẩm, hãy nhắc AI sử dụng nó làm tham chiếu hình ảnh cho sản phẩm.\n" +
"7. CHẤT LƯỢNG HÌNH ẢNH (STYLE): Luôn bao gồm cụm từ: 'Commercial Cinematic 8k, high-end product photography style'.\n" +
"8. TỐI GIẢN & SÚC TÍCH: Viết prompt ngắn gọn, tập trung vào hành động chính. Tránh dùng từ ngữ rườm rà. Tổng độ dài prompt không quá 150 từ.\n" +
"9. CẤU TRÚC PROMPT:\n" +
"    - Location: (Ngắn gọn)\n" +
"    - Time: (Ngắn gọn)\n" +
"    - Style: Commercial Cinematic 8k, high-end product photography style\n" +
"    - Action: (Mô tả hành động nhân vật tương tác với sản phẩm theo mốc thời gian. Giữ thoại và tên sản phẩm tiếng Việt).\n" +
"10. CẤM MÁU VÀ PHỤ ĐỀ: Tuyệt đối không mô tả máu (blood/gore) and không chèn chữ/phụ đề. Sử dụng cụm từ: 'no on-screen text or subtitles'.\n" +
JIMENG_SAFETY_GUIDELINES;
...
export const suggestStoryIdea = async (theme: StoryTheme, subTheme?: string): Promise<string> => {
  const ai = getAiClient();
  ...
  const subThemeText = subTheme ? ` với nội dung cụ thể: "${subTheme}"` : "";
  const response = await ai.models.generateContent({
    model: "gemini-3-flash-preview",
    contents: [{ parts: [{ text: `Hãy gợi ý một ý tưởng kịch bản phim ngắn cực hay, gây cấn và cảm động cho chủ đề: "${themeNames[theme]}"${subThemeText}. Ý tưởng phải có mở đầu, diễn biến và một cái kết ấn tượng.` }] }],
    config: {
      thinkingConfig: { thinkingLevel: ThinkingLevel.HIGH }
    }
  });
...
export const extractCharactersFromIdea = async (idea: string): Promise<IdeaCharacter[]> => {
  const ai = getAiClient();
  const response = await ai.models.generateContent({
    model: "gemini-3-flash-preview",
    contents: [{ parts: [{ text: `Dựa trên ý tưởng kịch bản sau: "${idea}". Hãy liệt kê danh sách các nhân vật chính xuất hiện trong ý tưởng này.
Trả về định dạng JSON là một mảng các đối tượng có cấu trúc: { "name": string, "gender": "male" | "female", "outfit": string }.
Lưu ý: "outfit" là mô tả trang phục phù hợp với bối cảnh ý tưởng.` }] }],
    config: {
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.ARRAY,
  ...
  try {
    ...
    console.error("Failed to parse characters from idea", e);
    return [];
  }
};
...
export const refineStoryIdea = async (
  theme: StoryTheme, 
  originalIdea: string, 
  characters: IdeaCharacter[]
): Promise<string> => {
  const ai = getAiClient();
  const charContext = characters.map(c => `- ${c.name} (${c.gender === 'male' ? 'Nam' : 'Nữ'}): ${c.outfit}`).join('\n');
  
  const response = await ai.models.generateContent({
    model: "gemini-3-flash-preview",
    contents: [{ parts: [{ text: `Hãy viết lại ý tưởng kịch bản sau: "${originalIdea}".
Yêu cầu:
1. Giữ nguyên cốt truyện chính nhưng điều chỉnh các nhân vật theo danh sách sau:
${charContext}
2. Đảm bảo các tình tiết phù hợp với giới tính và trang phục đã chọn.
3. Ý tưởng mới phải hấp dẫn, gây cấn và có cái kết ấn tượng.` }] }],
    config: {
      thinkingConfig: { thinkingLevel: ThinkingLevel.HIGH }
    }
  });
  return response.text || originalIdea;
};
...
export const developStoryScript = async (
  ...
  idea: string,
  numEpisodes: number,
  durationPerEpisode: number, // Now in seconds
  product?: { name: string, description: string, noDialogue?: boolean },
  characters?: IdeaCharacter[]
): Promise<StoryScript> => {
  const ai = getAiClient();
  const numScenesPerEpisode = Math.ceil(durationPerEpisode / 12);

  const productContext = product ? `\nSẢN PHẨM CẦN QUẢNG CÁO: "${product.name}" - ${product.description}. Hãy lồng ghép sản phẩm này vào câu chuyện một cách tinh tế.` : "";
  const dialogueContext = product?.noDialogue ? "\nLƯU Ý: Đây là video KHÔNG THOẠI. Hãy tập trung vào ngôn ngữ hình thể, biểu cảm khuôn mặt và hành động tương tác với sản phẩm thay vì lời thoại." : "";
  const characterContext = characters && characters.length > 0 
    ? `\nDANH SÁCH NHÂN VẬT CỐ ĐỊNH:\n${characters.map(c => `- ${c.name} (${c.gender === 'male' ? 'Nam' : 'Nữ'}): ${c.outfit}`).join('\n')}\nSử dụng ĐÚNG tên, giới tính và trang phục này trong kịch bản.`
    : "";

  const prompt = `Bạn là một biên kịch chuyên nghiệp. Dựa trên chủ đề "${theme}" và ý tưởng: "${idea}".${productContext}${dialogueContext}${characterContext}
Hãy phát triển thành một bộ phim gồm ${numEpisodes} tập. Mỗi tập dài ${durationPerEpisode} giây.

YÊU CẦU:
1. Viết tiêu đề và tóm tắt tổng thể cho bộ phim.
2. Với mỗi tập, hãy viết tiêu đề, tóm tắt nội dung.
3. Chia mỗi tập thành ${numScenesPerEpisode} cảnh quay chi tiết (mỗi cảnh 12 giây). Mô tả cảnh phải ngắn gọn, súc tích (dưới 50 từ).
...
7. LỒNG GHÉP SẢN PHẨM: Nếu có sản phẩm, hãy đảm bảo nó xuất hiện ít nhất 2-3 lần trong mỗi tập phim.
8. TỐI GIẢN: Ưu tiên sự ngắn gọn nhưng vẫn đầy đủ ý nghĩa cốt truyện.`;
...
export const generateStoryPrompt = async (
  ...
  if (characters && characters.length > 0) {
    ...
    if (activeCharacters.length > 0) {
      ...
      activeCharacters.slice(0, 3).forEach(char => {
        const genderStr = char.gender === 'female' ? 'Nữ' : 'Nam';
        const outfitStr = char.customOutfit ? char.customOutfit : 'Trang phục theo cameo';
        characterInstructions += `- ${char.name} (${genderStr}): ${outfitStr}\n`;
      });
    }
  }

  const productInstructions = product ? `\nSẢN PHẨM: ${product.name}. Mô tả: ${product.description}.` : "";

  const cameoParts = characters
    .map(c => {
      const outfit = c.customOutfit ? c.customOutfit : "original cameo outfit";
      return `${c.name.toUpperCase()} is a ${c.gender === 'female' ? 'woman' : 'man'} wearing ${outfit}`;
    });
  
  const uniqueCameoParts = Array.from(new Set(cameoParts));
  const cameoPrefix = `Characters: ${uniqueCameoParts.join('. ')}. lower body wearing black pants and black shoes.`;

  const promptText = `HÃY TẠO PROMPT VIDEO AI NGẮN GỌN CHO CHỦ ĐỀ: ${theme.toUpperCase()}\n\n` +
    ...
    `YÊU CẦU CẤU TRÚC (VIẾT CỰC KỲ NGẮN GỌN):\n` +
    `- Location: ...\n` +
    `- Time: ...\n` +
    `- Style: ${['sales', 'review', 'tvc'].includes(theme) ? 'Commercial Cinematic 8k, high-end product photography style' : 'Hollywood Cinematic 8k, ultra-detailed'}\n` +
    `- Action: (${anyCameo ? "BẮT ĐẦU bằng câu: '" + cameoPrefix + "' " : ""}${product?.noDialogue ? "TUYỆT ĐỐI KHÔNG CÓ LỜI THOẠI. Tập trung vào ngôn ngữ hình thể, biểu cảm khuôn mặt và hành động tương tác với sản phẩm." : "Mô tả hành động vật lý lồng ghép lời thoại trực tiếp [TÊN] [hành động nói]: '...'"} theo mốc 0-3s, 3-6s, 6-9s, 9-12s. GIỮ NGUYÊN LỜI THOẠI VÀ TÊN SẢN PHẨM TIẾNG VIỆT. no on-screen text or subtitles. TỐI ĐA 100 TỪ CHO PHẦN NÀY.)\n\n` +
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      thinkingConfig: { thinkingLevel: ThinkingLevel.HIGH },
      systemInstruction: theme === 'sales' ? SALES_STUDIO_INSTRUCTION : STORY_STUDIO_INSTRUCTION,
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.OBJECT,
        properties: {
...
`;
      ...
      topMain.forEach((char) => {
        ...
        const cameoNote = (char.useCameoOutfit && !char.customOutfit) ? " [CHẾ ĐỘ CAMEO ĐANG BẬT - GIỮ NGUYÊN TRANG PHỤC GỐC]" : 
                         (char.useCameoOutfit && char.customOutfit) ? " [CHẾ ĐỘ CAMEO ĐANG BẬT - SỬ DỤNG MÔ TẢ RIÊNG]" : "";
        const customOutfitNote = char.customOutfit ? ` [MÔ TẢ TRANG PHỤC: ${char.customOutfit}]` : "";
        ...
        characterInstructions += "- " + char.name + charDesc + cameoNote + customOutfitNote + refImageNote + ": Đây là nhân vật chính. " + 
          (char.useCameoOutfit ? (char.customOutfit ? `SỬ DỤNG TRANG PHỤC NÀY: ${char.customOutfit}. ` : "GIỮ NGUYÊN TRANG PHỤC CAMEO GỐC. ") : 
      ...
      if (anyCameo) {
        const cameoParts = characters
          .filter(c => c.useCameoOutfit)
          .map(c => c.customOutfit ? `${c.name.toUpperCase()} is wearing ${c.customOutfit}` : "keep original cameo outfit");
        
        const uniqueCameoParts = Array.from(new Set(cameoParts));
        const cameoPrefix = `Same cameo character. ${uniqueCameoParts.join('. ')}. lower body wearing black pants and black shoes.`;
    ...
    contents: [
      {
        parts: [
          ...
          {
            text: "Hãy phân tích video này. Trích xuất từng câu thoại kèm theo số giây (timestamp) bắt đầu của cảnh đó. Sau đó đưa ra một cốt truyện (plot) tổng thể cho video này. Trả về định dạng JSON."
          }
        ]
      }
    ],
        ...
        properties: {
          ...
          scenes: {
            ...
            items: {
              ...
              required: ["timestamp", "dialogue", "description"]
            }
          }
        },
        required: ["plot", "scenes"]
      }
    }
  });
  return JSON.parse(response.text || "{}");
