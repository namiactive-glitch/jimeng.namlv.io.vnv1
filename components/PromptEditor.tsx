...
const PromptEditor: React.FC<PromptEditorProps> = ({ prompt, onUpdate, sceneId }) => {
  ...
  // Auto-sync logic
  useEffect(() => {
    if (!isEditing || !autoSyncEnabled || !editedPrompt.translation.trim()) return;
    
    // Only sync if the translation has changed from the original or last synced version
    if (editedPrompt.translation === prompt.translation && editedPrompt.prompt === prompt.prompt) return;
  ...
  }, [editedPrompt.translation, isEditing, autoSyncEnabled]);
          ...
          {isEditing && (
            <div className="flex items-center gap-2 mr-2">
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                <span className="ml-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Auto Sync</span>
              </label>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="bg-orange-50/50 border border-orange-200 p-5 rounded-2xl relative group transition-all hover:border-orange-300">
          <div className="flex justify-between items-center mb-3">
            <span className="text-[10px] font-black text-orange-600 uppercase tracking-[0.2em] block">Prompt Tiếng Việt (Jimeng)</span>
            <button 
          ...
          {isEditing ? (
            ...
            <p className="text-sm text-orange-900 leading-relaxed font-medium italic px-1">
              {editedPrompt.translation || <span className="text-orange-300 font-normal">Chưa có prompt tiếng Việt...</span>}
            </p>
          )}
        </div>
        ...
        <div className="bg-orange-50/30 border border-orange-100 p-5 rounded-2xl relative group transition-all hover:border-orange-200">
          <div className="flex justify-between items-center mb-3">
            <span className="text-[10px] font-black text-orange-400 uppercase tracking-[0.2em] block">Prompt Tiếng Trung (Jimeng/Veo)</span>
            <button 
          ...
          {isEditing ? (
            <textarea
              ...
              placeholder="Prompt Tiếng Trung..."
            />
          ) : (
            <p className="text-sm text-orange-800 leading-relaxed font-medium px-1">
              {editedPrompt.chinesePrompt || <span className="text-orange-200 font-normal">Chưa có prompt tiếng Trung...</span>}
            </p>
          )}
        </div>
      </div>
